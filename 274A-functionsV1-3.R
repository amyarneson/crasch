...
####################################################################################################
# Creates tables for each item with classical item statistics and others a la CMap and ConQuest
# Note that the output is saved as .txt because .csv will not work since each table could be a different size
# requires packages: WrightMap, ltm

# data is the response data in WIDE format (must have item names as colnames())
# item.params is the matrix of item estimates + SEs in BASSR format (ConQuest parameterization)
# item.fit is the tam.fit()$itemfit object
# output = c("concise","")

Rasch.itemanalysis <- function(data=wide,item.params=item.pars,
                               item.thres=thresholds,item.fit=fit$itemfit,
                               BASSR.item=item.info,BASSR.cons=cons.info,
                               pers.rawprop=pers.pars[,3]/pers.pars[,4],
                               pers.params=pers.pars[,1],
                               empty.categs=empties,
                               outfile="itemanalysis",writeout=Autosave) {
  require(ltm)

  # create the byitem table
  byitem = cbind(Item.no = 1:ncol(data),
                 Count = colSums(!is.na(data)),
                 Missing = colSums(is.na(data)),
                 Estimate = item.params[,1],
                 SE_est = item.params[,(ncol(item.params)/2+1)], #should work for dichotomous or polytomous
                 item.fit[1:ncol(data),c(2:4,6:8)]
  )

  # POLYTOMOUS bystep table:
  if (ncol(item.thres)>1) {
    # create the bystep table (for polytomous items)
    bystep = matrix(nrow=length(which(BASSR.item[,6:ncol(BASSR.item)]==1)),ncol=10)
    colnames(bystep) = c("Item","Cat","Count","Score","Estimate","SE_est","Thres",
                         "Pt.Bs","MeanAbil","SD_MeanAbil")
    start = 1
    for(i in 1:ncol(wide)) {
      K = sum(BASSR.item[i,6:ncol(BASSR.item)])
      fill = c(start:(start+K-1))
      # item and category names
      bystep[fill,1] = rep(BASSR.item$Item.Name[i],K)
      bystep[fill,2] = as.matrix(BASSR.cons[1,5:ncol(BASSR.cons)][which(BASSR.item[i,6:ncol(BASSR.item)]==1)])
      # identify empty categories & fill their frequencies as 0
      if (length(empty.categs[[i]])>0) {
        bystep[fill[(bystep[fill,2] %in% empty.categs[[i]])],3] = 0
        fill = fill[!(bystep[fill,2] %in% empty.categs[[i]])]
      }
      # frequencies
      bystep[fill,3] = table(wide[,i])
      # numerical scores
      bystep[fill,4] = 0:(length(fill)-1)
      # estimates
      bystep[fill[-1],5] = item.params[i,2:(length(fill))]
      # standard errors of estimates - 0 category and highest will not have SEs
      bystep[fill[-c(1,length(fill))],6] = item.params[i,(ncol(item.params)/2+2):(ncol(item.params)/2+length(fill)-1)]
      # thresholds
      bystep[fill[-1],7] = item.thres[i,1:(length(fill)-1)]
      # point biserials
      bystep[fill,8] = sapply(fill,function(x) {
        -biserial.cor(pers.rawprop[!is.na(data[,i])],
                      data[!is.na(data[,i]),i]==bystep[x,4])
      })
      bystep[fill,9] = sapply(fill,function(x) {
        mean(pers.params[data[!is.na(data[,i]),i]==bystep[x,4]],na.rm=TRUE)
      })
      bystep[fill,10] = sapply(fill,function(x){
        sd(pers.params[data[!is.na(data[,i]),i]==bystep[x,4]],na.rm=TRUE)
      })

      start = start+K
    }
  } else if (ncol(item.thres==1)) {
    # create the bystep table (for dichotomous items)
    bystep = matrix(nrow=2*nrow(BASSR.item),ncol=10)
    colnames(bystep) = c("Item","Cat","Count","Score","Estimate","SE_est","Thres",
                         "Pt.Bs","MeanAbil","SD_MeanAbil")
    ...
    for(i in 1:ncol(wide)) {
      fill = c(2*i-1,2*i)
      # item and category names
      bystep[fill,1] = rep(as.character(BASSR.item$Item.Name[i]),2)
      bystep[fill,2] = as.matrix(BASSR.cons[1,5:ncol(BASSR.cons)][which(BASSR.item[i,6:ncol(BASSR.item)]==1)])
      ...
      bystep[fill,4] = 0:1
      # estimates
      bystep[fill[2],5] = item.params[i,1]
      # standard errors of estimates
      bystep[fill[2],6] = item.params[i,2]
      # thresholds
      bystep[fill[2],7] = item.thres[i,]
      # point biserials
      bystep[fill,8] = sapply(fill,function(x) {
        -biserial.cor(pers.rawprop[!is.na(data[,i])],
                      data[!is.na(data[,i]),i]==as.numeric(bystep[x,4]))
      })
      bystep[fill,9] = sapply(fill,function(x) {
        mean(pers.params[data[!is.na(data[,i]),i]==bystep[x,4]],na.rm=TRUE)
      })
  ...
  if(writeout) {
    write.csv(byitem,paste0(outfile,"-item.csv"))
    write.csv(bystep,paste0(outfile,"-step.csv"))
  }
